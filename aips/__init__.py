...
from engines.solr import (
    SolrLTR,
    SolrSemanticKnowledgeGraph,
    SolrEntityExtractor,
    SolrSparseSemanticSearch,
)
...
def get_executing_notebook_path():
    return globals().get(
        "__vsc_ipynb_file__",  # only exists during a remote vscode kernel
...
def render_search_results(query, results):
    ...
    with open(search_results_template_file) as file:
        ...
        results_template_syntax = (
            "<!-- BEGIN_TEMPLATE: SEARCH_RESULTS -->(.*)<!-- END_TEMPLATE: SEARCH_RESULTS[^>]*-->"
        )
        x = re.search(results_template_syntax, file_content, flags=re.S)
        results_template = x.group(1)

        separator_template_syntax = (
            "<!-- BEGIN_TEMPLATE: SEPARATOR -->(.*)<!-- END_TEMPLATE: SEPARATOR[^>]*-->"
        )
        ...
        for result in results:
            ...
            rendered += (
                results_template.replace("${NAME}", result.get("name", "UNKNOWN"))
...
def fetch_products(doc_ids):
    request = {
        "query": " ".join([str(id) for id in doc_ids]),
        "query_fields": ["upc"],
        "limit": len(doc_ids),
    }
    ...
    df.insert(
        0,
        "image",
        df.apply(lambda row: '<img height="100" src="' + img_path_for_product(row) + '">', axis=1),
    )
    return df
...
def render_judged(products, judged, grade_col="ctr", label=""):
    ...
    return HTML(
        style
        + f"<h1>{label}</h>"
        + w_prods.to_html(float_format=lambda x: "%10.4f" % x, escape=False)
    )
